...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (typeof requestedTab === 'string' && requestedTab !== activeTab) {
  ...
  const dashboardTabs = [
    ...
    { id: 'bulk-email', label: 'Bulk Email', icon: Mail, route: '/bulk-email' },
              ...
              {dashboardTabs.map((tab) => {
                ...
                return (
                  <button
                    key={tab.id}
                    type="button"
                    onClick={() => tab.route ? router.push(tab.route) : setActiveTab(tab.id)}
