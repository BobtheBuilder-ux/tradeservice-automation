...
const defaultReturnTo = '/settings/company?calendly=connected';
...
async function inputBody(req: Request): Promise<any> { try { return await req.json(); } catch { return {}; } }
function safeReturnTo(value?: string) {
  if (!value || typeof value !== 'string' || !value.startsWith('/') || value.startsWith('//') || value.includes('\\')) return defaultReturnTo;
  try {
...
function addConnectionStatus(path: string, status: 'connected' | 'error', message?: string) {
  ...
  parsed.searchParams.set('platform', 'calendly');
