...
function buildAgentPrompt(context: JsonRecord) {
  ...
  const businessHoursStart = String(context.tenant?.business_hours_start || '10:00').slice(0, 5);
  const businessHoursEnd = String(context.tenant?.business_hours_end || '17:00').slice(0, 5);
  ...
  return [
    ...
    `When runtime variables or action context show email_first_followup_call=true, this is the call after the first outreach email. Vary the wording naturally, but keep this meaning: "Hello, my name is ${agentName}, assistant for ${tenantName}. We saw you filled one of our forms about {{service_interest}}. The details are in the email, and we would like to schedule a meeting with you and our expert advisor." If you reach voicemail, leave that reason for calling and mention the email details.`,
    ...
    `Business-hours guardrail: outbound voice calls are allowed only from ${businessHoursStart} to ${businessHoursEnd} in the tenant timezone (${tenantTimezone}). Never ask the system to place or retry a voice call outside that local tenant window. If outside that window, wait until the next allowed window or use an allowed non-call channel only when the lead consent permits it.`,
...
function buildConversationConfig(context: JsonRecord, documents: JsonRecord[], toolIds: string[] = []) {
  ...
  return {
    agent: {
      first_message: `Hi, I’m ${agentName}. You filled our form on insurance, and I see you’re interested in {{service_interest}}. Would you like to book a consultation with one of our experts?`,
      prompt: {
...
function elevenLabsToolDefinitions() {
  return [
    ...
    webhookToolConfig(
      'check_availability',
      'Check the tenant booking path or return a booking link only when the lead has not provided a date/time. Do not use this before create_booking during a live call when the lead already gave day and time.',
      {
    ...
    webhookToolConfig(
      'create_booking',
      'Create a booking immediately once the lead agrees to a date and time. During live calls, use this directly after the lead gives day/time; do not call check_availability first.',
      {
