...
- The local `backend/` app is retired. Do not add new product code, routes,
  services, workers, tests, Docker config, or deploy logic there.
- New product work belongs in `frontend/`, `functions/`, `migrations/`,
  and InsForge SDK/RPC/Functions.

## Invariants — Never Violate These

- Frontend product reads/writes use InsForge SDK/RPC through `frontend/lib/insforge*.js` helpers.
- Provider callbacks, OAuth callbacks, webhook validation, queue execution,
  live/test actions, and privileged provider calls live in `functions/`.
- Database/schema changes live in `migrations/` and are applied through InsForge.
- Do not restore `frontend/lib/api.js`, Next rewrites to a local API server,
  or `NEXT_PUBLIC_BACKEND_URL` / `NEXT_PUBLIC_API_URL` product dependencies.
- Do not create new Express routes or local backend workers.
- Runtime secrets belong in InsForge secrets; local env files are for development only.
- Every tenant-owned read/write includes tenant context.
- Human tenant users live in `tenant_users`; AI workers live in `tenant_agents`.
